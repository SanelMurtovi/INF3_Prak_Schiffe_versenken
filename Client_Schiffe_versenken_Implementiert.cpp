...
private:
    ...
    int boardSizeX;
    int boardSizeY;
    vector<vector<bool> > visited; // To track visited fields
...
public:
    ...
    BattleShipClient(string address, int port, int boardX, int boardY)
        ...
        visited.resize(boardSizeX, vector<bool>(boardSizeY, false));
    }
    ...
    // This method generates shots using a cluster strategy
    void generateClusterShots() {
        shots.clear();
        visited.assign(boardSizeX, vector<bool>(boardSizeY, false)); // Reset visited fields

        // Random start point
        int startX = rand() % boardSizeX;
        int startY = rand() % boardSizeY;

        vector<pair<int, int> > toVisit;
        toVisit.push_back(make_pair(startX, startY));

        while (!toVisit.empty()) {
            pair<int, int> current = toVisit.back();
            toVisit.pop_back();

            int x = current.first;
            int y = current.second;

            if (x < 0 || x >= boardSizeX || y < 0 || y >= boardSizeY || visited[x][y]) {
                continue;
            }

            // Mark as visited and add to shots
            visited[x][y] = true;
            shots.push_back(make_pair(x + 1, y + 1)); // Convert to 1-based index

            // Add neighboring cells (cluster exploration)
            toVisit.push_back(make_pair(x + 1, y)); // Right
            toVisit.push_back(make_pair(x - 1, y)); // Left
            toVisit.push_back(make_pair(x, y + 1)); // Down
            toVisit.push_back(make_pair(x, y - 1)); // Up
        }
    }
...
int main() {
    ...
    if (strategyChoice == "1") {
        ...
        selectedStrategy = &BattleShipClient::generateClusterShots;
    } else {
